      ...
      await fetch(`https://stonks.co.zw/api/news/${query}`)
        ...
        .then((json) => {
          json["rss"].channel.item.forEach((article) => {
            temp[Number.parseInt(new Date(article.pubDate).getTime() / 1000)] =
              article;
          });
...
const News = (props) => {
  if (Object.keys(props.news)[0] === props.counter) {
    return Object.keys(props.news[props.counter])
      .sort((a, b) => b - a)
      .map((article, index) => {
        return (
          <NewsLink key={index}>
            <a
              href={props.news[props.counter][article].link}
              target="_blank"
              rel="noreferrer"
            >
              {props.news[props.counter][article].title}
            </a>
            <time>
              Posted{" "}
              {formatDistance(
                new Date(props.news[props.counter][article].pubDate),
                new Date(),
                { addSuffix: true }
              )}
            </time>
...
const App = () => {
  ...
  const [today] = useState(
    new Intl.DateTimeFormat("en-GB", { month: "long", day: "numeric" }).format(
      Date.now()
    )
  );
      ...
      const fetchData = async () => {
        await fetch(`https://api.stonks.co.zw/history/${counter}`)
          ...
          .then((json) => {
            ...
            if (
              Object.keys(updatedHistory).length ===
    ...
    let data = Object.keys(counters).map((counter, index) => {
      return {
        ...
        percent_change: Number(
          counters[counter].percent_change.replace("%", "")
        ),
        color:
          Number(counters[counter].percent_change.replace("%", "")) === 0
            ? "#2f323d"
            : Number(counters[counter].percent_change.replace("%", "")) > 0
            ? "#65c466"
            : "#ea4e3d",
        ...
        <Listing>
          {Object.keys(counters).map((counter, index) => {
            ...
            return (
              <ListItem
                key={index}
                query={counters[counter].company_name.replaceAll(" ", "+")}
                setNews={setNews}
                counter={counter}
              >
                <summary>
                    ...
                    >
                      <defs>
                        <linearGradient id="pos" x1="0" y1="0" x2="0" y2="1">
                          <stop
                            offset="5%"
                            stopColor="rgba(118, 214, 113, 0.65)"
                          />
                          <stop
                            offset="95%"
                            stopColor="rgba(118, 214, 113, 0)"
                          />
                        </linearGradient>
                        <linearGradient id="neg" x1="0" y1="0" x2="0" y2="1">
                          <stop
                            offset="5%"
                            stopColor="rgba(234, 78, 61, 0.65)"
                          />
                  ...
                  <div className="price_change">
                    <span className="closing_price">
                      ${counters[counter].closing_price}
                    </span>
                    <span className={`percent_change ${direction}`}>
                      {counters[counter].percent_change}
                    </span>
                  </div>
                </summary>
                <NewsList>
